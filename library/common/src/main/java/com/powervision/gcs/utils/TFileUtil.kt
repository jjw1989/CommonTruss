...
object TFileUtil {
     ...
     * @return
     */
    fun getCecheFolder(context: Context): File {
    ...
    fun getSDCardPath(): String {
        return if (!isSDCardEnable()) "sdcard unable!" else Environment.getExternalStorageDirectory().path + File.separator
    }
    ...
    fun getDataPath(): String {
        return if (!isSDCardEnable()) "sdcard unable!" else Environment.getDataDirectory().path
    }
    ...
    fun getFreeSpace(): String {
        ...
        return byteToFileSize(availableBlocks * blockSize)
    }
    /**
     * 字节数转合适大小
     * @param byteNum 字节数
     */
    fun byteToFileSize(byteNum:Long ):String{
        if (byteNum<0){
            return "shouldn't be less than zero!"
        }else if (byteNum<KB){
            return String.format(Locale.getDefault(), "%.3fB", byteNum.toDouble())
        }else if(byteNum<MB){
            return String.format(Locale.getDefault(), "%.3fKB", byteNum.toDouble() / KB)
        }else if (byteNum<GB){
            return String.format(Locale.getDefault(), "%.3fMB", byteNum.toDouble() / MB)
        }else{
            return String.format(Locale.getDefault(), "%.3fGB", byteNum.toDouble() / GB)
        }
    }
    /**
     * SD卡是否可用.
    ...
    fun getDiskFileDir(context: Context): String {
        var cachePath: String? = null
        if (Environment.MEDIA_MOUNTED == Environment.getExternalStorageState() || !Environment.isExternalStorageRemovable()) {
            cachePath = context.getExternalFilesDir(Environment.DIRECTORY_MOVIES)!!.path
        } else {
        ...
        try {
            ...
            while (reader.readLine() != null) {
                ...
                if (line.length > 0 && line.startsWith("http://")) {
                    ...
                    buf.append("file:" + pathList[num].absolutePath + "\r\n")
                    num++
                } else {
                    buf.append(line + "\r\n")
                }
    ...
    fun getFileByPath(filePath: String): File? {
        return if (TStringUtil.isNullString(filePath)) null else File(filePath)
    }
    //==============================================================================================
    ...
    fun writeFileFromIS(file: File?, inputStream: InputStream?, append: Boolean): Boolean {
        ...
        try {
            ...
            var len: Int
            while ( inputStream.read(data, 0, KB) != -1) {
                len = inputStream.read(data, 0, KB)
                os.write(data, 0, len)
            }
            return true
        } catch (e: IOException) {
    ...
    fun readFile2List(file: File?, st: Int, end: Int, charsetName: String): List<String>? {
        ...
        try {
            ...
            if (TStringUtil.isNullString(charsetName)) {
    ...
    fun input2OutputStream(inputStream: InputStream?): ByteArrayOutputStream? {
        ...
        try {
            ...
            val b = ByteArray(KB)
            var len: Int
            while (inputStream.read(b, 0, KB) != -1) {
    ...
    fun getFileLines(file: File?): Int {
        ...
        try {
            ...
            var readChars: Int
            while ( input.read(buffer, 0, KB) != -1) {
                readChars = input.read(buffer, 0, KB)
     ...
     * 获取文件大小
     *
     * 例如：getFileSize(file, RxConstUtils.MB); 返回文件大小单位为MB
     *
     * @param file 文件
     * @return 文件大小
     */
    fun getFileSize(file: File?): String {
        return if (!isFileExists(file)) "" else byteToFileSize(file!!.length())
    }
...
//    }
    ...
    fun getDirName(filePath: String): String {
        if (TStringUtil.isNullString(filePath)) return filePath
    ...
    fun getFileExtension(filePath: String): String {
        ...
        val lastPoi = filePath.lastIndexOf('.')
        val lastSep = filePath.lastIndexOf(File.separator)
        return if (lastPoi == -1 || lastSep >= lastPoi) "" else filePath.substring(lastPoi)
    }
...
//        return File(RxPhotoUtils.getImageAbsolutePath(context, uri))
//    }
